<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h3 class="text-lg font-bold text-fg-primary">宗门管理</h3>
      <div class="flex space-x-2">
        <AppButton variant="primary" size="sm" @click="fetchAll">刷新</AppButton>
      </div>
    </div>

    <!-- 宗门统计卡片 -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-if="statsLoading" class="col-span-full text-center text-fg-faint py-4">统计加载中...</div>
      <div v-else-if="sectStats.length === 0" class="col-span-full text-center text-fg-faint py-4">暂无宗门数据</div>
      <!-- 单个宗门统计卡片 -->
      <div
        v-for="stat in sectStats"
        :key="stat.sect_id"
        class="bg-surface-raised rounded-panel border border-line p-4 hover:border-line-strong transition-colors cursor-pointer"
        :class="{ 'ring-2 ring-gold-500': filterSectId === stat.sect_id }"
        @click="filterBySect(stat.sect_id)"
      >
        <div class="flex justify-between items-start mb-2">
          <h4 class="text-base font-bold text-gold-500">{{ stat.sect_name }}</h4>
          <span class="text-xs text-fg-faint num">{{ stat.sect_id }}</span>
        </div>
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            <div class="text-fg-muted text-xs">成员数</div>
            <div class="text-fg-primary font-bold num">{{ stat.member_count }}</div>
          </div>
          <div>
            <div class="text-fg-muted text-xs">总贡献度</div>
            <div class="text-green-400 font-bold num">{{ stat.total_contribution }}</div>
          </div>
          <div>
            <div class="text-fg-muted text-xs">长老数</div>
            <div class="text-purple-400 font-bold num">{{ stat.elder_count }}</div>
          </div>
          <div>
            <div class="text-fg-muted text-xs">弟子数</div>
            <div class="text-blue-400 font-bold num">{{ stat.disciple_count }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div class="flex flex-wrap justify-between items-center gap-2">
        <h4 class="text-base font-bold text-fg-primary">成员列表</h4>
        <div class="flex gap-2 items-center">
          <label class="text-sm text-fg-muted">宗门筛选：</label>
          <select
      ...
      <div class="bg-surface-base/50 rounded-panel border border-line overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <!-- 加载中 -->
              <tr v-if="membersLoading" class="text-center text-fg-faint">
                <td colspan="9" class="px-3 py-6">加载中...</td>
              </tr>
              <!-- 空数据 -->
              <tr v-else-if="members.length === 0" class="text-center text-fg-faint">
              ...
              >
                <td class="px-3 py-2 text-fg-muted num">{{ m.player_id }}</td>
                <td class="px-3 py-2 text-fg-primary">{{ m.nickname }}</td>
                <td class="px-3 py-2 text-gold-500">{{ m.sect_name }}</td>
                <td class="px-3 py-2 text-green-400 font-bold num">{{ m.contribution }}</td>
                <td class="px-3 py-2">
                  <span
                ...
                <td class="px-3 py-2 text-fg-muted text-xs whitespace-nowrap num">{{ formatDate(m.joined_at) }}</td>
                <td class="px-3 py-2 text-fg-muted text-xs whitespace-nowrap num">{{ formatDate(m.last_check_in) }}</td>
                <td class="px-3 py-2 text-fg-muted text-xs whitespace-nowrap num">{{ formatDate(m.last_transfer) }}</td>
                <td class="px-3 py-2 text-center whitespace-nowrap">
                  <AppButton variant="outline" size="xs" class="mr-1" @click="openContributionModal(m)">贡献</AppButton>
                  <AppButton variant="outline" size="xs" class="mr-1" @click="openRoleModal(m)">身份</AppButton>
                  <AppButton variant="danger" size="xs" @click="handleKick(m)">踢出</AppButton>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div class="bg-surface-base rounded-panel border border-line p-6 w-full max-w-md shadow-2xl shadow-black/60">
        <h3 class="text-lg font-bold text-fg-primary mb-4">调整宗门贡献度</h3>
        <div v-if="contributionModal.member" class="space-y-4">
          <p class="text-fg-secondary">
            玩家：<span class="text-gold-500">{{ contributionModal.member.nickname }}</span>
            （{{ contributionModal.member.sect_name }}）
          </p>
          <p class="text-fg-muted text-sm num">当前贡献度：<span class="text-green-400">{{ contributionModal.member.contribution }}</span></p>
          <div>
            <label class="block text-sm text-fg-muted mb-1">新贡献度数值</label>
            <input
            ...
            >
            <p class="mt-1 text-xs text-fg-faint">提示：贡献度不能为负数，设置为 0 相当于清空贡献</p>
          </div>
        </div>
        <div class="flex justify-end space-x-2 mt-6">
          <AppButton variant="default" @click="closeContributionModal">取消</AppButton>
          <AppButton
            variant="primary"
            :disabled="contributionModal.submitting"
            @click="confirmContribution"
          >{{ contributionModal.submitting ? '提交中...' : '确认' }}</AppButton>
        </div>
      </div>
    </div>
    ...
    >
      <div class="bg-surface-base rounded-panel border border-line p-6 w-full max-w-md shadow-2xl shadow-black/60">
        <h3 class="text-lg font-bold text-fg-primary mb-4">设置宗门身份</h3>
        <div v-if="roleModal.member" class="space-y-4">
          <p class="text-fg-secondary">
            玩家：<span class="text-gold-500">{{ roleModal.member.nickname }}</span>
            （{{ roleModal.member.sect_name }}）
          </p>
          <p class="text-fg-muted text-sm">当前身份：<span :class="roleModal.member.role === 'elder' ? 'text-purple-300' : 'text-blue-300'">{{ formatRole(roleModal.member.role) }}</span></p>
          <div>
            <label class="block text-sm text-fg-muted mb-1">新身份</label>
            <select
        ...
        <div class="flex justify-end space-x-2 mt-6">
          <AppButton variant="default" @click="closeRoleModal">取消</AppButton>
          <AppButton
            variant="primary"
            :disabled="roleModal.submitting"
            @click="confirmRole"
          >{{ roleModal.submitting ? '提交中...' : '确认' }}</AppButton>
        </div>
      </div>
    </div>
  </div>
...
const confirmContribution = async () => {
  ...
  try {
    ...
    uiStore.showApiError(err, '调整贡献度失败')
  } finally {
    contributionModal.submitting = false
  }
}
...
const confirmRole = async () => {
  ...
  try {
    ...
    uiStore.showApiError(err, '设置身份失败')
  } finally {
    roleModal.submitting = false
  }
}
...
const handleKick = (member) => {
  ...
  emit('showConfirm', title, message, async () => {
    try {
      ...
      uiStore.showApiError(err, '踢出宗门失败')
    }
  })
}

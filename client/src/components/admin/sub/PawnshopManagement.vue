  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex space-x-2">
        <AppButton variant="primary" size="sm" @click="fetchList(pagination.page)">刷新当票</AppButton>
    ...
    <div v-if="metrics" class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div class="bg-surface-raised rounded-lg border border-line p-3">
        <div class="text-xs text-fg-muted mb-1">活跃当票</div>
        <div class="text-2xl font-bold text-amber-400 num">{{ metrics.listings.active }}</div>
      ...
      <!-- 今日典当 -->
      <div class="bg-surface-raised rounded-lg border border-line p-3">
        <div class="text-xs text-fg-muted mb-1">今日典当</div>
        <div class="text-2xl font-bold text-amber-400 num">{{ metrics.today.pawn_count }} 笔</div>
      ...
      <!-- 今日赎回 -->
      <div class="bg-surface-raised rounded-lg border border-line p-3">
        <div class="text-xs text-fg-muted mb-1">今日赎回</div>
        <div class="text-2xl font-bold text-emerald-400 num">{{ metrics.today.redeem_count }} 笔</div>
      ...
      <!-- 信用分布 -->
      <div class="bg-surface-raised rounded-lg border border-line p-3">
        <div class="text-xs text-fg-muted mb-1">信用分布</div>
        <div class="text-2xl font-bold text-amber-400 num">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <input
            ...
            min="1"
            placeholder="按玩家ID筛选"
            class="px-3 py-1 text-sm w-40 bg-surface-sunken border border-line rounded-control text-fg-secondary focus-ring focus:border-gold-600"
    ...
    <div class="bg-surface-base rounded-lg border border-line overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <!-- 空数据 -->
            <tr v-else-if="list.length === 0" class="text-center text-fg-faint">
            ...
            >
              <td class="px-3 py-2 text-fg-muted num">{{ item.id }}</td>
              <td class="px-3 py-2">
                <div class="text-fg-primary">{{ item.player_nickname }}</div>
                <div class="text-xs text-fg-faint">ID: {{ item.player_id }} · {{ item.player_realm }}</div>
              </td>
              <td class="px-3 py-2">
                <div class="text-amber-300">{{ item.item_name }}</div>
                <div class="text-xs text-fg-faint">{{ item.item_quality }}</div>
              </td>
              <td class="px-3 py-2 text-fg-secondary num">{{ item.quantity }}</td>
              ...
              <td class="px-3 py-2 text-xs text-fg-muted num">{{ formatDate(item.pawned_at) }}</td>
              <td class="px-3 py-2 text-center whitespace-nowrap">
                <button
                  @click="openDetailModal(item)"
                  class="px-2 py-1 bg-blue-600 hover:bg-blue-500 rounded-control text-fg-primary text-xs mr-1"
                >详情</button>
                <AppButton variant="primary" size="xs" v-if="item.status === 'active'" @click="openForceRedeemModal(item)" class="mr-1">
                  强制赎回
                </AppButton>
                <AppButton variant="danger" size="xs" v-if="item.status === 'active'" @click="openCancelModal(item)">
                  取消当票
                </AppButton>
              </td>
            </tr>
      ...
      <div class="px-4 py-3 border-t border-line flex items-center justify-between text-sm">
        ...
        <div class="flex items-center gap-2">
          <AppButton variant="default" size="sm" :disabled="pagination.page <= 1 || loading" @click="fetchList(pagination.page - 1)">
            上一页
          </AppButton>
          <span class="text-fg-secondary num">{{ pagination.page }} / {{ pagination.totalPages }}</span>
          <AppButton variant="default" size="sm" :disabled="pagination.page >= pagination.totalPages || loading" @click="fetchList(pagination.page + 1)">
            下一页
          </AppButton>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <div class="flex items-center gap-2">
          ...
          <input
            ...
            min="1"
            placeholder="玩家ID"
            class="px-3 py-1 text-sm w-32 bg-surface-sunken border border-line rounded-control text-fg-secondary focus-ring focus:border-gold-600"
          />
        </div>
        <div class="flex items-center gap-2">
          <label class="text-sm text-fg-muted whitespace-nowrap">信用值：</label>
          <input
            v-model.number="creditForm.credit"
            type="number"
            min="0"
            placeholder="0-100"
            class="px-3 py-1 text-sm w-24 bg-surface-sunken border border-line rounded-control text-fg-secondary focus-ring focus:border-gold-600"
          />
        </div>
        <div class="flex items-center gap-2 flex-1 min-w-[200px]">
          ...
          <input
            ...
            class="flex-1 px-3 py-1 text-sm bg-surface-sunken border border-line rounded-control text-fg-secondary focus-ring focus:border-gold-600"
          />
        </div>
        <AppButton variant="primary" size="sm" :disabled="operating" @click="openCreditConfirmModal">
          提交调整
        </AppButton>
      </div>
    ...
    <Modal :isOpen="!!detailData" title="当票详情" width="640px" @close="detailData = null">
      <div v-if="detailData" class="space-y-3 text-sm">
        <!-- 当票基础信息 -->
        <div class="bg-surface-sunken rounded p-3 border border-line">
          <div class="text-xs text-fg-muted mb-2">当票信息</div>
          <div class="grid grid-cols-2 gap-x-3 gap-y-2">
            <div>
              <label class="block text-fg-faint mb-1">当票ID</label>
              <div class="text-fg-primary num">{{ detailData.listing.id }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">状态</label>
              <div>
            ...
            <div>
              <label class="block text-fg-faint mb-1">物品</label>
              <div class="text-amber-300">{{ detailData.listing.item_name }} ({{ detailData.listing.item_quality }})</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">数量</label>
              <div class="text-fg-primary num">{{ detailData.listing.quantity }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">典当所得</label>
              <div class="text-amber-400 font-bold">{{ detailData.listing.pawn_amount }} 灵石</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">手续费</label>
              <div class="text-rose-400">{{ detailData.listing.pawn_fee }} 灵石</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">赎回价</label>
              <div class="text-rose-400 font-bold">{{ detailData.listing.redeem_amount }} 灵石</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">基础价值</label>
              <div class="text-fg-secondary">{{ detailData.listing.base_price }} 灵石</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">典当时间</label>
              <div class="text-fg-primary text-xs num">{{ formatDate(detailData.listing.pawned_at) }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">赎回截止</label>
              <div class="text-fg-primary text-xs num">{{ formatDate(detailData.listing.redeem_deadline) }}</div>
            </div>
            <div v-if="detailData.listing.redeemed_at">
              <label class="block text-fg-faint mb-1">实际赎回时间</label>
              <div class="text-emerald-400 text-xs num">{{ formatDate(detailData.listing.redeemed_at) }}</div>
            </div>
          </div>
        </div>

        <!-- 玩家信息 -->
        <div v-if="detailData.player" class="bg-surface-sunken rounded p-3 border border-line">
          <div class="text-xs text-fg-muted mb-2">玩家信息</div>
          <div class="grid grid-cols-2 gap-x-3 gap-y-2">
            <div>
              <label class="block text-fg-faint mb-1">玩家ID</label>
              <div class="text-fg-primary num">{{ detailData.player.id }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">昵称</label>
              <div class="text-fg-primary">{{ detailData.player.nickname }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">境界</label>
              <div class="text-fg-primary">{{ detailData.player.realm }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">当铺信用</label>
              <div class="text-amber-400 font-bold">{{ detailData.player.pawnshop_credit }}</div>
            </div>
            <div>
              <label class="block text-fg-faint mb-1">灵石余额</label>
        ...
        <!-- 关联历史记录 -->
        <div v-if="detailData.histories && detailData.histories.length > 0" class="bg-surface-sunken rounded p-3 border border-line">
          <div class="text-xs text-fg-muted mb-2">关联历史记录</div>
          <ul class="space-y-1 text-xs">
            <li
              v-for="h in detailData.histories"
              :key="h.id"
              class="flex items-center gap-2 px-2 py-1 rounded bg-surface-base"
            >
              <span :class="actionTypeTextClass(h.action_type)" class="font-bold">
                {{ actionTypeLabel(h.action_type) }}
              </span>
              <span class="text-fg-secondary">{{ h.item_name }} x{{ h.quantity }}</span>
              <span class="text-amber-400">{{ h.amount }} 灵石</span>
              <span class="text-fg-faint ml-auto num">{{ formatDate(h.created_at) }}</span>
            </li>
          </ul>
        </div>
      </div>
      <template #footer>
        <AppButton variant="default" @click="detailData = null">关闭</AppButton>
      </template>
    </Modal>
    ...
    <Modal :isOpen="forceRedeemModal.show" title="强制赎回当票" width="480px" @close="forceRedeemModal.show = false">
      <div v-if="forceRedeemModal.listing" class="space-y-3 text-sm">
        <p class="text-fg-secondary">确定要强制赎回以下当票吗？</p>
        <div class="bg-surface-sunken rounded p-3 border border-line space-y-2">
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">当票ID</span>
            <span class="text-fg-primary num">{{ forceRedeemModal.listing.id }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">玩家</span>
            <span class="text-fg-primary">{{ forceRedeemModal.listing.player_nickname }} ({{ forceRedeemModal.listing.player_id }})</span>
          </div>
          <div class="flex items-center justify-between">
      ...
      <template #footer>
        <AppButton variant="default" @click="forceRedeemModal.show = false">取消</AppButton>
        <AppButton variant="primary" :disabled="operating" @click="confirmForceRedeem">
    ...
    <Modal :isOpen="cancelModal.show" title="强制取消当票" width="480px" @close="cancelModal.show = false">
      <div v-if="cancelModal.listing" class="space-y-3 text-sm">
        <p class="text-fg-secondary">确定要强制取消以下当票吗？</p>
        <div class="bg-surface-sunken rounded p-3 border border-line space-y-2">
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">当票ID</span>
            <span class="text-fg-primary num">{{ cancelModal.listing.id }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">玩家</span>
            <span class="text-fg-primary">{{ cancelModal.listing.player_nickname }} ({{ cancelModal.listing.player_id }})</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">物品</span>
      ...
      <template #footer>
        ...
        <AppButton variant="danger" :disabled="operating" @click="confirmCancel">
          {{ operating ? '执行中...' : '确认取消' }}
        </AppButton>
      </template>
    </Modal>
    ...
    <Modal :isOpen="creditConfirmModal.show" title="确认调整信用" width="420px" @close="creditConfirmModal.show = false">
      <div class="space-y-3 text-sm">
        <p class="text-fg-secondary">请确认以下信用调整操作：</p>
        <div class="bg-surface-sunken rounded p-3 border border-line space-y-2">
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">玩家ID</span>
            <span class="text-fg-primary">{{ creditForm.player_id }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">新信用值</span>
            <span class="text-amber-400 font-bold">{{ creditForm.credit }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-fg-faint">原因</span>
            <span class="text-fg-primary text-xs">{{ creditForm.reason || '-' }}</span>
          </div>
        </div>
        <p class="text-xs text-amber-400">
      ...
      <template #footer>
        <AppButton variant="default" @click="creditConfirmModal.show = false">取消</AppButton>
        <AppButton variant="primary" :disabled="operating" @click="confirmUpdateCredit">
          {{ operating ? '执行中...' : '确认调整' }}
        </AppButton>
      </template>
    </Modal>
...
const fetchMetrics = async () => {
  try {
    ...
    uiStore.showApiError(err, '[PawnshopManagement] 获取指标失败')
  }
}
...
const confirmForceRedeem = async () => {
  ...
  try {
    ...
    uiStore.showApiError(err, '强制赎回失败')
  } finally {
...
const confirmCancel = async () => {
  ...
  try {
    ...
    uiStore.showApiError(err, '取消当票失败')
  } finally {
    operating.value = false
  }
}
...
const confirmUpdateCredit = async () => {
  ...
  try {
    ...
    uiStore.showApiError(err, '信用调整失败')
  } finally {
    operating.value = false
  }
}
...
const statusBadgeClass = (status) => {
  const map = {
    ...
    auctioned: 'bg-surface-hover text-fg-secondary'
  }
...
const actionTypeTextClass = (actionType) => {
  const map = {
    ...
    auction: 'text-fg-muted'
  }
  return map[actionType] || 'text-fg-muted'
}

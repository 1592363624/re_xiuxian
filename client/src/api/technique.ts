...
/** 获取功法总览（已习得 + 可习得 + 系统设置） */
export const getTechniqueList = () => apiClient.get(`${BASE}/list`)

/** 习得 / 研习指定功法 */
export const learnTechnique = (techniqueId: string) =>
  apiClient.post(`${BASE}/learn`, { technique_id: techniqueId })

/** 修炼指定功法：提升熟练度与修为 */
export const practiceTechnique = (techniqueId: string) =>
  apiClient.post(`${BASE}/practice`, { technique_id: techniqueId })

/** 突破指定功法：消耗资源升阶 */
export const breakthroughTechnique = (techniqueId: string) =>
  apiClient.post(`${BASE}/breakthrough`, { technique_id: techniqueId })

/** 针对指定功法领悟神通 */
export const comprehendTechnique = (techniqueId: string) =>
  apiClient.post(`${BASE}/comprehend`, { technique_id: techniqueId })
...
export const equipTechnique = (techniqueId: string, slot?: 'main' | 'auxiliary' | null) =>
  apiClient.post(`${BASE}/equip`, { technique_id: techniqueId, slot: slot ?? null })

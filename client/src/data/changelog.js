...
export const currentVersion = 'v0.5.3_BETA'; // 🔔 发布新版时，请修改此版本号以触发用户弹窗
...
export const changelog = [
  {
    ...
    sections: [
      {
        title: '新增功能：傀儡工坊系统（大衍诀·控傀解锁·制造出战护法·淬炼维修回收·战斗属性加成）',
        type: 'feature',
        items: [
          '【系统定位】傀儡工坊是大衍诀第三层·控傀解锁的后期系统（玩法文档第23节），通过参悟图谱、消耗材料制造傀儡，为 PVP/PVE 战斗提供属性加成，或设置护法在闭关被袭时自动反击。',
          '【五种傀儡】机关木傀（稀有/均衡）、铁甲战傀（史诗/重防）、五行灵傀（史诗/法术）、影傀（传说/极速突袭，大衍诀4层·千机解锁）、大衍灵傀（神话/全属性卓越，大衍诀5层·衍神解锁）。',
          '【图谱参悟】获得图谱物品后参悟解锁制造权限，5种图谱来源差异化：LDC商城/昆吾山/苍坤洞府/虚天殿/青元子世界Boss，促进多人副本协作。',
          '【制造机制】消耗灵石+材料制造傀儡，需大衍诀层数+已学图谱双重校验，事务+行级锁保证并发安全，BigInt 安全处理灵石消耗。',
          '【出战加成】出战傀儡按 battle_stat_ratio（30%）提供攻防血速属性加成，已集成到 CombatService 的战斗初始化（HP加成）+ 普通攻击（ATK加成）+ 技能释放（ATK加成）三处。',
          '【护法反击】护法傀儡闭关被袭时自动反击，造成 guard_counter_ratio（50%）攻击力伤害，PuppetService.getGuardPuppetCounter 供 SeclusionService/PvpService 调用。',
          '【淬炼升级】消耗灵石（base+per_level×当前等级）+机关核心，属性按 8%/级增长（速度3%/级），成功率随等级递减（100%→50%下限），失败材料消耗但等级不变，每次成功消耗2点耐久，最高20级。',
          '【维修耐久】消耗灵石（50/点）+机关核心×1，恢复满耐久度，耐久为0时无法出战/护法/淬炼。',
          '【回收机制】仅闲置傀儡可回收，二步确认（预览→确认），材料返还50%+灵石返还30%（含淬炼投入估算），事务保证原子性。',
          '【数据库】migration_0072 创建 player_puppets + player_puppet_blueprints 两张表，幂等性检查，自动执行。',
          '【配置中心化】puppet_data.json 完整配置（5种傀儡/5种图谱/淬炼/维修/回收/战斗加成/WebSocket事件），全可热更新。',
          '【物品系统】item_data.json 新增 6 种材料（机关核心/铁甲片/五行灵晶/影石/大衍灵晶/上古养魂木）+ 5 种图谱物品，修复 ancient_soul_wood 物品缺失 bug。',
          '【后端接口】新增 11 个 API：配置/工坊/参悟图谱/制造/出战/护法/取消/淬炼/维修/回收预览/确认回收。',
          '【后端服务】PuppetService（约 550 行）15 个方法，事务+行级锁+BigInt安全+WebSocket推送全覆盖。',
          '【前端 API】puppet.ts 新增 11 个 API 方法 + 完整 TypeScript 接口定义，类型覆盖配置/工坊/制造/淬炼/维修/回收/出战/护法。',
          '【前端 UI】PuppetPanel.vue 三 Tab 面板（工坊/制造/说明）+ 傀儡卡片（属性/耐久条/状态徽章）+ 参悟/制造/淬炼/回收四次二次确认 Modal + 结果 Modal，琥珀金属主题体现机关术质感；ActionBar 新增傀儡按钮，GameLayout 注册面板。',
  ...
  },
  {
        ...
        sections: [
          {
            title: '新增功能：大衍诀修炼系统（五层修炼·神识联动·飞升前置）',
            type: 'feature',
            items: [
              '【系统定位】大衍诀是后期成长线（玩法文档第23节），核心价值是提高神识操控、傀儡运用、裂缝探索、节点稳定和飞升前置能力，与神识淬炼系统深度联动。',
              '【五层修炼】凝识→分念→控傀→千机→衍神，每层提供神识倍率（×1.30/×1.60/×2.00/×2.45/×3.00），影响探查、斗法神识威压、御宝预算等判定。',
              '【参悟机制】每日 3 次参悟，冷却 10 分钟，消耗修为（随层数递增：500+300×层级）获得大衍诀经验，跨日重置次数，需达到炼气期（rank≥15）方可参悟。',
              '【突破机制】经验满后消耗对应层数残篇尝试突破，成功率随层数递减（80%→70%→60%→50%→40%，下限30%），成功则层数+1经验重置，失败则残篇损耗但经验保留。',
              '【神识联动】大衍诀层数直接影响神识上限公式：divine_sense_max = 100 + (realm_rank-14)×50 + dayan_level×100，已在 DivineSenseService 中集成验证。',
              '【飞升前置】五层·衍神是飞升灵界的必要条件，通过 /dayan/ascension-check 接口可随时检查是否满足。',
              '【数据库】migration_0071 为 player_ascension 表追加 3 字段（dayan_meditate_count/last_meditate_at/last_meditate_date），复用已有 dayan_level+dayan_exp 字段，无需新建表。',
              '【配置中心化】dayan_data.json 完整配置（启用开关/境界要求/5层参数/5种残篇/参悟参数/突破参数/飞升前置/神识加成/WebSocket事件），全可热更新。',
              '【物品系统】item_data.json 新增 4 种残篇（凝识/分念/控傀/衍神残篇，epic~mythic 品质），千机残篇已存在，5 种残篇覆盖全部 5 层突破需求。',
              '【后端接口】新增 5 个 API：配置/状态/参悟/突破/飞升前置检查，所有写操作使用事务+行级锁保证并发安全。',
              '【后端服务】DayanService（约 450 行）5 个公开方法，BigInt 安全处理修为消耗，WebSocket 推送参悟/突破结果。',
              '【前端 API】dayan.ts 新增 10+ TypeScript 接口 + 5 个 API 方法，类型完整覆盖配置/状态/参悟/突破/飞升检查。',
              '【前端 UI】DayanPanel.vue 三 Tab 面板（修炼状态/层级图鉴/玩法说明）+ 参悟/突破二次确认 Modal + 突破结果 Modal，靛蓝主题体现神识修炼的玄奥感；ActionBar 新增大衍诀按钮，GameLayout 注册面板。',
